...
public class Parser {
    ...
    public static double evaluate(String expression) {
        if (!Tokenizer.chekcingCorrectBrakets(expression)) {
            throw new IllegalArgumentException("invalid number of brackets");
        }
        if (!Tokenizer.checkingCorrectOperators(expression)) {
            throw new IllegalArgumentException("invalid number of operators");
        }
        if (!Tokenizer.checkingValidChar(expression)) {
            throw new IllegalArgumentException("invalid characters");
        }
        List<String> tokens = Tokenizer.brakeInTokens(expression);
